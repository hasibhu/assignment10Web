...
const SwitzerlandAll = () => {
    const [switzerlandLocations, setSwitzerlandLocations] = useState([]);
    ...
    useEffect(() => {
        fetch('http://localhost:3000/myLocation/')
            ...
            .then(data => {
                ...
                const italyData = data.filter(item => item.countryName === 'Switzerland');
                setSwitzerlandLocations(italyData);
            })
    ...
    return (
        <div>
            <h1>Locations in Switzerland {switzerlandLocations.length}</h1>
